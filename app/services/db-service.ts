...
export const getDBConnection = async () => await SQLite.openDatabaseAsync('farm-app.db');
...
export const getCrops  = async (db: SQLite.SQLiteDatabase) => {
  try {
    const results = await db.getAllAsync('SELECT rowid as id, name, description FROM crops');
